...
class BaseLogic(ABC):
    ...
    def _compute_frequency(self) -> dict[int, float]:
        """역대 당첨번호 출현 빈도 계산. 없으면 균등 확률 반환."""
        freq = {n: 0 for n in range(LOTTO_MIN, LOTTO_MAX + 1)}
    ...
    def _expected_freq(self) -> float:
        """번호당 기댓값 출현 횟수."""
        return len(self.historical_draws) * LOTTO_PICK / LOTTO_MAX

    def _get_cold_numbers(self, lookback: int = 30) -> set[int]:
        """최근 lookback회 동안 한 번도 안 나온 번호 집합."""
        recent = self.historical_draws[-lookback:] if self.historical_draws else []
        recent_set = {n for draw in recent for n in draw}
        return set(range(LOTTO_MIN, LOTTO_MAX + 1)) - recent_set
...
class AntiPopularLogic(BaseLogic):
    ...
    def score_combination(self, combo: list[int]) -> float:
        ...
        if self.historical_draws:
            ...
            expected = self._expected_freq()
            if expected > 0:
                rarity_bonus = sum(
                    max(0.0, (expected - freq[n]) / expected) for n in combo
                ) * 0.6  # 역대 덜 나온 번호일수록 보너스

        return base_score + sum_bonus + rarity_bonus
...
class WangPreferenceAvoidanceLogic(BaseLogic):
    ...
    def _recent_winning_overlap(self, combo: list[int], lookback: int = 30) -> int:
        """최근 N회 당첨번호와의 중복 개수. [강화] lookback 5→30회."""
    ...
    def score_combination(self, combo: list[int]) -> float:
        ...
        # 최근 30회 당첨번호 1~3개 포함 시 보너스 (자주 플레이어가 회피)
        recent_overlap = self._recent_winning_overlap(combo, lookback=30)
        if 1 <= recent_overlap <= 3:
            score += 1.5
        ...
        # [강화] 장기 미출현 번호 보너스 (최근 30회 안 나온 번호)
        if self.historical_draws:
            cold = self._get_cold_numbers(lookback=30)
            cold_in_combo = sum(1 for n in combo if n in cold)
            score += cold_in_combo * 1.2  # 미출현 번호 포함할수록 가산
        ...
        if self.historical_draws:
            ...
            if expected > 0:
                score += sum(
                    (expected - freq[n]) / expected * 0.4 for n in combo
                )
...
class PolinSpatialLogic(BaseLogic):
    ...
    def score_combination(self, combo: list[int]) -> float:
        ...
        # [강화] 최근 20회 미출현 번호 보너스
        absence_bonus = 0.0
        if self.historical_draws:
            cold = self._get_cold_numbers(lookback=20)
            absence_bonus = sum(0.8 for n in combo if n in cold)

        # [강화] 역대 출현 빈도가 낮은 번호 위치 가중
        freq_bonus = 0.0
        if self.historical_draws:
            freq = self._compute_frequency()
            expected = self._expected_freq()
            if expected > 0:
                freq_bonus = sum(
                    max(0.0, (expected - freq[n]) / expected) * 0.5
                    for n in combo
                )

        return pos_score + diversity_bonus + absence_bonus + freq_bonus
...
class BakerConsciousLogic(BaseLogic):
    ...
    def _build_farrell_probability(self) -> dict[int, float]:
        ...
        q_i = base + 생일효과 + 럭키효과 + 위치효과 + [강화] 최근 빈도 효과
        최근에 많이 나온 번호 → 플레이어들이 더 많이 선택(핫넘버 심리) → q_i ↑ → 회피 대상
        """
        q = {}
        for n in range(LOTTO_MIN, LOTTO_MAX + 1):
            value = 1.0
        ...
        # [강화] 최근 20회 빈도로 q_i 보정
        # 핫 번호 = 플레이어들이 더 많이 선택 → q_i 상향 → 분배 인원 증가 → 회피 대상
        if self.historical_draws:
            freq = self._compute_frequency()
            expected = self._expected_freq()
            recent_freq = {n: 0 for n in range(LOTTO_MIN, LOTTO_MAX + 1)}
            recent_draws = self.historical_draws[-20:]
            for draw in recent_draws:
                for n in draw:
                    if LOTTO_MIN <= n <= LOTTO_MAX:
                        recent_freq[n] += 1
            recent_expected = len(recent_draws) * LOTTO_PICK / LOTTO_MAX
            if recent_expected > 0:
                for n in q:
                    hotness = recent_freq[n] / recent_expected  # 1.0 = 평균, >1 = 핫
                    q[n] *= (1.0 + (hotness - 1.0) * 0.3)  # 30% 반영
                    q[n] = max(q[n], 0.01)
